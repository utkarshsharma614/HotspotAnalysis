...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  val actualpointsSum0 = sumofPoints.first().getLong(1);
  val actualpointsSum1 = sumofPoints.first().getDouble(2);
  val actualpointsSum2 = sumofPoints.first().getLong(0);

  val mean =(actualpointsSum0.toDouble / numCells.toDouble).toDouble;
  val SD = math.sqrt(((actualpointsSum1.toDouble / numCells.toDouble) - (mean.toDouble * mean.toDouble))).toDouble
